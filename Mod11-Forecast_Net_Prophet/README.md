...
## Background
This project constructs a program to analyze e-commerce company MercadoLibre to seek predictive growth avenues for the company's stock by observing user data. Mining search traffic data for unusual patterns and seasonality factors to find some coorelation with historical stock price movement over time are utilized to make predictable forecast models. The forecasts uses time series model utilizing FaceBook Prophet for non-linear trends  are observed with seasonal effects and historical data.  

The increasing growth in young e-commerce companies is becoming increasingly more competitive not only with their industry peers, but also with public investment financially with s  in cryptocurrency investing has made this asset class popular for portfolios because they expand diversification in the hi-risk aggressive growth asset classes. Utilizing python and unsupervised machine learning code, this program uses ‘K-Means’ algorithms to group cryptocurrencies in clusters to analyze performance in different time periods. The time performance results are then revealed visually to assist with investment decisions of cryptocurrencies for portfolios.
